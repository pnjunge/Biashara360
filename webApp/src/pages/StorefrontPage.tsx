  ...
  return (
    <main className="storefront-shell">
      <header className="store-hero">
        <div><p className="store-eyebrow">Online store</p><h1>{store.businessName}</h1><p>{store.welcomeMessage}</p></div>
...
const STORE_STYLES = `
  ...
  .store-hero { padding: 42px max(24px, calc((100vw - 1180px)/2)); color: white; background: linear-gradient(120deg,#073b36,#0f766e); display:flex; align-items:center; justify-content:space-between; gap:24px; }
  ...
  .store-categories button { border:1px solid #dbe5e3; background:white; border-radius:999px; padding:8px 14px; cursor:pointer; white-space:nowrap; } .store-categories button.active { background:#0f766e; color:white; border-color:#0f766e; }
  ...
  .store-product-image { background:linear-gradient(135deg,#e7f4f1,#f8fbfa); display:flex; align-items:center; justify-content:center; color:#0f766e; } .store-product-image img { width:100%; height:100%; object-fit:cover; }
  .store-product-body { padding:17px; } .store-product-body span { color:#0f766e; text-transform:uppercase; font-size:10px; font-weight:800; letter-spacing:1px; } .store-product-body h2 { font-size:18px; margin:6px 0; } .store-product-body p { font-size:13px; line-height:1.5; color:#61706e; min-height:39px; } .store-product-body strong { font-size:18px; }
  ...
  .store-primary { border:0; border-radius:10px; padding:13px 18px; background:#0f766e; color:white; font-weight:800; cursor:pointer; width:100%; } .store-primary:disabled { opacity:.55; cursor:not-allowed; }
  ...
  @media(max-width:560px){ .store-products{grid-template-columns:1fr}.store-layout{padding:20px 14px 40px}.store-hero{padding:30px 18px}.store-product{grid-template-rows:130px 1fr auto} }
`
