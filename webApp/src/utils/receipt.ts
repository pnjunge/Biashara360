...
export function printOrderReceipt(order: OrderResponse, profile: BusinessProfileResponse | null, isProForma = false) {
  ...
  printWindow.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Receipt ${escapeHtml(order.orderNumber)}</title>
    <style>
      ...
      * { box-sizing: border-box; } body { width: 72mm; margin: 0 auto; color: #111; font: 11px/1.35 ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; }
      ...
      <div class="rule"></div><div class="center message"><strong>${escapeHtml(profile?.receiptHeader || (isUnpaid ? 'PLEASE PRESENT BILL TO CASHIER FOR SETTLEMENT' : 'THANK YOU FOR YOUR BUSINESS!'))}</strong><br>${escapeHtml(profile?.receiptFooter || '')}</div><div class="center muted" style="margin-top:9px">Powered by Biashara360</div>
