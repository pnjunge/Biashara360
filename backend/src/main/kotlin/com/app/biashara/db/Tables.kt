...
object BusinessesTable : Table("businesses") {
    ...
    val storefrontThemeColor = varchar("storefront_theme_color", 7).default("#0F766E")
    val storefrontHeadline = varchar("storefront_headline", 120).default("Shop with us online")
    val storefrontDescription = varchar("storefront_description", 500).default("")
    val storefrontBannerUrl = text("storefront_banner_url").nullable()
    val storefrontLayout = varchar("storefront_layout", 20).default("GRID")
...
object UsersTable : Table("users") {
    ...
    val loginPinHash = varchar("login_pin_hash", 255).nullable()
...
object OrdersTable : Table("orders") {
    ...
    val taxIncluded = bool("tax_included").default(false)
    val taxRate = double("tax_rate").default(0.0)
...
object HospitalityTablesTable : Table("hospitality_tables") {
    ...
    val isActive = bool("is_active").default(true)
    val waiterUserId = varchar("waiter_user_id", 36).nullable()
    val mergedIntoTableId = varchar("merged_into_table_id", 36).nullable()
    val positionX = integer("position_x").default(0)
    val positionY = integer("position_y").default(0)
    val shape = varchar("shape", 20).default("RECTANGLE")
    val createdAt = timestamp("created_at")
    val updatedAt = timestamp("updated_at")
    override val primaryKey = PrimaryKey(id)
}
